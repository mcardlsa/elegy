...
class Funeral extends CI_Controller {
        ...
        if($this->user_auth->is_logged_in())
        {
	        ...
	        if($this->form_validation->run())
	        {
	            ...
	            if(!empty($_POST['event1'][0]))
	            {
	            	...
	            	$t =strtotime($_POST['event1'][1]);
	            	$time =date('Y-m-d H:i:s',$t);
	            	$param['event'][0]['time']=$time;
    ...
    public function update_funeral($id)
    {
        ...
        if($this->user_auth->is_logged_in())
        {
	        ...
	        $this->form_validation->set_rules('serviceType', 'serviceType', 'required|trim|xss_clean');
	        $this->form_validation->set_rules('dateOfBirth', 'DOB', 'trim|xss_clean');
	        $this->form_validation->set_rules('dateOfDeath', 'DOD', 'trim|xss_clean');
	        $this->form_validation->set_rules('familymember', 'Family Member', 'xss_clean');
	       	$this->form_validation->set_rules('pallBearers', 'Pall Bearers', 'xss_clean');
	...
	*/
    ...
    public function gallery()
    {
            ...
            $view_data1=$this->session->userdata('userInfo');
	    	$fields['funeralId']=$view_data1->funeralId;
	    	if($this->user_auth->is_logged_in()) {
				$fields['userId']=$view_data1->userId;
				$data['userId']=$fields['userId'];
				$data['hostId']=$this->Funeral_model->get_HostId($fields['funeralId']);
   			}
	...
	*/
	...
	{  
		...
		if ($this->user_auth->is_logged_in())
	    {
		    ...
		    $data['funeralId']=$view_data->funeralId;
		    $data['id']=$id;
			$data=$this->Funeral_model->delete_gallery($data);
            if($data['status']==200){
