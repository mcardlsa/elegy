...
class Funeral extends REST_Controller
{
 ...
 /**
	...
	*/
	...
	{ 
		...
		if(! mandatory_params_present($mandatory_key,$data,$missing_param))
        {
        	...
        	$result=$this->Funeral_model->get_funeral_detail_by_invitationCode($data);
			if($result) {
				$response['funerals']= $result;
				$response['message'] = "Success";
            	$response['status'] = "200";
	...
	* @return	HTTP response JSON
	*/
	function funeral_details_by_id_post()
	{ 
		$x		= stripslashes($this->input->post('data'));
        $data	= json_decode($x,true);
		...
		if(! mandatory_params_present($mandatory_key,$data,$missing_param))
        {
        	...
        	$result=$this->Funeral_model->get_funeral_detail_by_funeral_id($data['funeralId']);
			if($result) {
				$response['funerals']= $result;
				$response['message'] = "Success";
            	$response['status'] = "200";
	...
	*/
	function add_funeral_post()
	{
		...
		$mandatory_key = array('userToken','name','serviceType','dateOfBirth','dateOfDeath','familyMember','pallBearers','guest' );
		if(empty($_FILES['imageUrl']['name'])) {
			$response['message'] = "Image not found.";
			$response['status'] = "201";
			$this->response($response, 200); // 200 being the HTTP response code
	...
	*/
	function update_funeral_post()
	{
		$x		= stripslashes($this->input->post('data'));
        $data	= json_decode($x,true);
		$mandatory_key = array('userToken','funeralId','name','serviceType','dateOfBirth','dateOfDeath','familyMember','pallBearers' );
	...
	*/
	...
	
	{
		...
		$mandatory_key = array('userToken','feedId','funeralId');
	...
	* @return	HTTP response JSON
	*/
	function delete_gallery_post()
	
	{
		$x		= stripslashes($this->input->post('data'));
        $data	= json_decode($x,true);
		
		$mandatory_key = array('userToken','id','funeralId');
		...
		if(! mandatory_params_present($mandatory_key,$data,$missing_param))
			...
			else
			{
				...
				if($userTokenStatus['status'] == 200)
				{
					$data['userId'] = $this->User_model->get_user_id($data['userToken']);
					$data['id']= $data['id'];
					$result = $this->Funeral_model->delete_gallery($data);
					if($result)
					{
						$this->response($result, 200); // 200 being the HTTP response code
					}else
